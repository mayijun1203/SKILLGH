...
#read NY PUMS data
NY<-readSelectCol("C:\\Users\\Wencong\\Desktop\\WFH\\COVID impact\\ACS\\PUMS\\psam_p36.csv",selectColumns)

# select workers who either live or work in NYC
worker<-subset(NY, ESR ==1 & (substr(PUMA,1, str_length(PUMA)-2) %in% c('37','38','39','40','41')|(POWSP==36& POWPUMA %in%c('3700','3800','3900','4000','4100'))
) )
...
ModebyPOW<-nyc%>%group_by(powBoro, Mode2, IndustryCluster)%>%summarise(Estimate=sum(PWGTP), MOE=sqrt(sum(MOE^2)))
ggplot(ModebyPOW, aes(fill=Mode2, y=Estimate, x=powBoro)) +  geom_bar(position="stack", stat="identity")+scale_fill_discrete(name="Commute Mode")+ggtitle("Means of Transportation by Place of Work Borough") +
  xlab("Place of Work") + ylab("Number of Workers")+ scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), limits = c(0, 2000000))+ scale_fill_brewer(palette="Set3")+geom_errorbar(aes(ymin=Estimate-MOE, ymax=Estimate+MOE), position="identity") 

#residence and workplace
OD<-nyc%>%group_by(resBoro, powBoro)%>%summarise(Estimate=sum(PWGTP), MOE=sqrt(sum(MOE^2)))
ggplot(OD, aes(fill=resBoro, y=Estimate, x=powBoro)) +  geom_bar(position="stack", stat="identity")+scale_y_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), limits = c(0, 320000))+scale_fill_brewer(palette="Set3")
